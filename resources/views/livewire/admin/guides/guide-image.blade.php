                                ...
                                <div class="row clearfix">
                                    @foreach ($images as $image)
                                        <div class="col-lg-3 col-md-4 col-sm-12">
                                            <div class="card">
                                                <a href="{{ url(env('GUIDE_IMAGES_PATCH') . $image->url) }}" class="file" target="_blank">
                                                    <div class="hover">
                                                        <form wire:submit="delete({{$image}})" style="display: inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                                            ...
                                                            <span class="file-name">
                                                                ...
                                                                <small class="mr-2"> تاریخ آپلود <span
                                                                        class="date">{{ Hekmatinasser\Verta\Verta::instance($image->created_at)->format('Y/n/j') }}</span></small>
                                                            </span>
                                                       
                                                        </div>
                                                    <div class="image">
                                                        <img src="{{ url(env('GUIDE_IMAGES_PATCH') . $image->url) }}"
