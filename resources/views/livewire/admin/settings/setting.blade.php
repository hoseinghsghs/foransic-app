                ...
                <form wire:submit="save" id="form_advanced_validation">
                    ...
                    <div class="row">
                            ...
                            @isset($emails)
                                @foreach($emails as $index=>$email)
                                    <div class="input-group mb-1" wire:key="email-{{$index}}">
                                        ...
                                        <div class="input-group-append">
                                            <button wire:click="removeEmail({{$index}})" wire:loading.attr="disabled"
                                                    wire:target="removeEmail({{$index}})" type="button"
                                                    class="btn btn-warning m-0"><i class="zmdi zmdi-delete"
                                                                                   wire:target="removeEmail({{$index}})"
                                                                                   wire:loading.remove></i>
                                                <span wire:loading wire:target="removeEmail({{$index}})"
                                                      class="spinner-border spinner-border-sm" role="status"
                                                      aria-hidden="true"></span>
                                            </button>
                                        </div>
                            ...
                            @isset($phones)
                                @foreach($phones as $index=>$phone)
                                    <div class="input-group mb-1" wire:key="phone-{{$index}}">
                                        ...
                                        <div class="input-group-append">
                                            <button wire:click="removePhone({{$index}})" wire:loading.attr="disabled"
                                                    wire:target="removePhone({{$index}})" type="button"
                                                    class="btn btn-warning m-0"><i wire:target="removePhone({{$index}})"
                                                                                   wire:loading.remove
                                                                                   class="zmdi zmdi-delete"></i>
                                                <span wire:loading wire:target="removePhone({{$index}})"
                                                      class="spinner-border spinner-border-sm" role="status"
                                                      aria-hidden="true"></span>
                                            </button>
                                        </div>
                        ...
                        <div class="form-group col-md-12" wire:ignore>
                            <label class="form-label">کلمات کلیدی سایت</label>
                            <input id="keywords" value="{{$seo_keywords}}" class="form-control" data-role="tagsinput">
                        </div>
                        <div class="form-group col-md-12">
                            <div class="form-line" wire:ignore>
                                <label for="privacy-summernote" class="form-label">حریم خصوصی</label>
                                <textarea id="privacy-summernote" class="summernote-editor">{{$site_privacy}}</textarea>
                            </div>
                        </div>
                        <div class="form-group col-md-12">
                            <div class="form-line" wire:ignore>
                                <label for="rules-summernote" class="form-label">قوانین و مقررات</label>
                                <textarea class="summernote-editor" id="rules-summernote">{{$site_rules}}</textarea>
                            </div>
                        </div>
                                    ...
                                    @foreach($links as $index=>$parent)
                                        <div class="panel panel-primary" wire:key="heading-{{$index}}">
                                                ...
                                                <h4 class="panel-title">
                                                    <div class="row no-gutters align-items-center">
                                                        <div class="col-10 col-md-6">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-auto">
                                                                    <button wire:click="addLink({{$index}})"
                                                                            ...
                                                                            type="button"><i
                                                                            wire:target="addLink({{$index}})"
                                                                            wire:loading.remove class="zmdi zmdi-hc-fw"></i>لینک
                                                                        <span wire:loading
                                                                              wire:target="addLink({{$index}})"
                                                                              class="spinner-border spinner-border-sm"
                                                                              role="status" aria-hidden="true"></span>
                                                                    </button>
                                                                    <button wire:click="removeGroupName({{$index}})"
                                                                            wire:loading.attr="disabled"
                                                                            wire:target="removeGroupName({{$index}})"
                                                                            class="btn btn-warning ml-2"
                                                                            data-collaps-id="${collaps_id}"
                                                                            type="button"><i class="zmdi zmdi-delete"
                                                                                             wire:target="removeGroupName({{$index}})"
                                                                                             wire:loading.remove></i>
                                                                        <span wire:loading
                                                                              wire:target="removeGroupName({{$index}})"
                                                        ...
                                                        <div class="col-2 col-md">
                                                            <a class="text-center text-md-left" role="button"
                                                               data-toggle="collapse" data-parent="#accordion_1"
                                                               href="#collapse_{{$index}}" aria-expanded="true"
                                            ...
                                            <div id="collapse_{{$index}}"
                                                 class="panel-collapse collapse bg-ghostwhite in show" role="tabpanel"
                                                    ...
                                                    @isset ( $parent['children'] )
                                                        @foreach ($parent['children'] as $key => $value)
                                                            <div class="input-group mb-2 mr-2"
                                                                ...
                                                                <div class="input-group-append">
                                                                    <button wire:click="removeLink({{$index}},{{$key}})"
                                                                            wire:loading.attr="disabled"
                        ...
                        <div class="form-group col-md-6">
                            <label class="form-label" for="exampleFormControlFile1">آپلود لوگوی سایت <span wire:loading
                                                                                                           wire:target="logo"
                                                                                                           class="spinner-border spinner-border-sm"
                                                                                                           role="status"
